...
export default function MerchantStorefrontPage() {
  ...
  const loadMerchantImageHistory = async (page = 1) => {
    try {
      ...
      const items: MerchantImageAsset[] = Array.isArray(data?.items)
        ? data.items
        : [];
      ...
      const message =
        error instanceof Error ? error.message : "Failed to load image history";
  ...
  return (
    <div className="min-h-screen bg-brand-hero p-4 md:p-8">
      <div className="mx-auto max-w-5xl space-y-6">
        <Link href="/" className="hidden md:block" aria-label="Go to home">
        ...
        <div className="rounded-3xl border border-white/70 bg-white/75 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.08)] p-6 md:p-8">
          ...
          <form onSubmit={onSave} className="mt-6 grid grid-cols-1 gap-5">
            <div
              ...
              className="rounded-2xl border border-brand-sand/50 bg-brand-parchment/40 px-4 py-4"
            >
              <p className="text-sm font-semibold text-brand-charcoal">
                Bulk Product Upload (CSV)
              </p>
              <p className="mt-1 text-xs text-brand-stone">
                1) Download the template. 2) Fill rows. 3) Preview CSV. 4) Import valid rows.
              </p>
                    ...
                    <tbody>
                      {bulkPreviewRows.slice(0, 8).map((row, idx) => (
                        <tr key={`${row.name}-${idx}`} className="border-t border-brand-sand/20">
                          <td className="px-2 py-2 text-brand-charcoal">{row.name}</td>
                          <td className="px-2 py-2 text-brand-charcoal">{row.category}</td>
                          <td className="px-2 py-2 text-brand-charcoal">₹{row.price}</td>
                          <td className="px-2 py-2 text-brand-charcoal">{row.stockQuantity}</td>
                          <td className="max-w-[220px] truncate px-2 py-2 text-brand-stone">
                            {row.imageUrl}
                          </td>
                        </tr>
                      ))}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-brand-stone">
                Upload product images to get public blob URLs. Images are stored with lossless compression and listed below.
              </p>
            </div>
